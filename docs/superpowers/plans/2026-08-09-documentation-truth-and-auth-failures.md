...
Expected: `tsc` без вывода; eslint — `0 errors`. Предупреждений должно стать **три**, а не четыре: `'error' is assigned a value but never used` из `auth/signin/route.ts` останется до Задачи 2, а из `callback` уйдёт, потому что там теперь используется `exchangeError`.

Если `authErrorMessage` в этом файле не используется — убрать его из импорта; текст показывает страница, а не маршрут.
