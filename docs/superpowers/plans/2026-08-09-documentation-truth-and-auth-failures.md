...
> **✅ ИСПОЛНЕНО 2026-08-09.** Все семь задач закрыты, каждая своим коммитом:
> `62d8124`, `3326c9c`, `6c4db2f`, `dac8811`, `91b25b3`, `3098a26`, `40ece21`.
> Итог на момент завершения: 113 юнитов, 45 тестов БД, 42 E2E, `tsc` чист,
> линтер без предупреждений, сборка проходит. Чек-боксы ниже оставлены
> незакрытыми намеренно — документ сохранён как он был согласован, чтобы
> расхождение между планом и исполнением было видно.
>
> **Три места, где план оказался неточен, и это выяснилось при исполнении:**
> 1. Задача 1, шаг 6 ожидал, что предупреждений линтера станет три. Их осталось
>    четыре: старый `callback` не деструктурировал результат вовсе, поэтому
>    неиспользуемой переменной там и не было. Исправлено в тексте плана.
> 2. Задача 1, шаг 7 искал `alert` по всей странице и поймал живую область Next
>    `__next-route-announcer__`: третий тест позеленел при лендинге, который
>    ничего не показывал — тест вхолостую того же класса, что A1. Локатор
>    переписан на поиск внутри `main`.
> 3. Задача 4 называла функцию ограничителя `check_rate_limit`; в миграции она
>    `rate_limit_hit`. В документ пошло настоящее имя.
>
> Дальнейшие шаги — [2026-08-10-next-steps.md](./2026-08-10-next-steps.md).
